			...
			if(json.bookList.length==0) {
				...
				for(var i = 0; i < json.bookList.length; i++) {
					...
					for(var j=0; j<items.length; j++) {
						...
						if(ename=="book_paper_price"||ename=="book_ebook_price"||ename=="book_ebook_dollar_price") {
							tableStr = tableStr + "<td>"+json.bookList[i][ename].toFixed(2)+"</td>";
						} else {
							tableStr = tableStr + "<td>"+json.bookList[i][ename]+"</td>";
						}
					}
					//tableStr = tableStr + "<td align='center' style='width:80px'><a class=\"text-info\" title=\"详细\" href=\"javascript:detail("+bookList.bookList[i].book_id+");\"><span class=\"glyphicon glyphicon-eye-open\"></span></a>&nbsp;&nbsp;<a class=\"text-success\" title=\"编辑\" href=\"javascript:edit('"+bookList.bookList[i].book_id+"');\"><span class=\"glyphicon glyphicon-edit\"></span></a>&nbsp;&nbsp;<a class=\"text-danger\" title=\"删除\" href=\"javascript:remove('"+bookList.bookList[i].book_id+"');\"><span class=\"glyphicon glyphicon-remove\"></span></a></td><tr>";
					tableStr = tableStr + "</tr>";
				}
...
function detail(bookId) {
	document.form.action="detailBook.action?bookId="+bookId;
...
function edit(bookId) {
	document.form.action="editBook.action?bookId="+bookId;
	document.form.submit();
}
...
function downFtpXml() {
	...
	if(exportXmlCol=="") {
		...
		$.ajax({
			...
			type:'post',
			async: false,
			data: {jsonString:myjson, exportXmlCol:exportXmlCol,localpath:localpath},
			dataType: 'json',
			success: function(data) {
				if(data!="-1") {
