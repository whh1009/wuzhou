...
function initAllUserName() {
	$.ajax({
		...
		success: function(data) {
			$("#userSel").empty();
			var userSel = "<option value='0'>请选择录入者</option>";
 ...
 * 获取图书信息
 */
function getAllBookList(page, searchType, searchContent, userId) {
	$.ajax({
		url:'getBookListByConditionAndUserId.action',
		type:'post',
		async: true,
...
function search() {
	...
	if(searchType=="检索类别") {
		...
		getAllBookList(1, searchType, searchContent, userId);
	}
}
/**
 * 初始化翻页
...
function prePage() {
	...
	getAllBookList(currentPage, searchType, searchContent, selUserId);
}
...
function firstPage() {
	getAllBookList(1, searchType, searchContent, selUserId);
}

function endPage() {
	getAllBookList(pageCount, searchType, searchContent, selUserId);
}
